...
export default function OrdersAdminPage() {
  ...
  >({});
  const [updatingTracking, setUpdatingTracking] = useState<
    Record<string, boolean>
  >({});
  const [cancellationReasons, setCancellationReasons] = useState<
    Record<string, string>
  >({});
  const [showCancellationInput, setShowCancellationInput] = useState<
    Record<string, boolean>
  >({});
  ...
  useEffect(() => {
    return () => {
      Object.values(trackingTimeouts).forEach((timeout) =>
    ...
    <div className="grid gap-6">
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Filter by status, update orders, export reports
          </CardDescription>
        </CardHeader>
        <CardContent className="grid gap-3">
                      ...
                      <td className="p-3">
                        <div className="text-xs">
                          <div className="font-medium">
                            {o.shippingMethod.replace("_", " ")}
                          </div>
                          {o.shippingFee > 0 && (
                            <div>Shipping: Rs. {o.shippingFee}</div>
                          )}
          ...
          <p className="text-xs text-neutral-500">
            Delivery tracking and notifications can be integrated with your
            courier APIs.
          </p>
        </CardContent>
      </Card>
