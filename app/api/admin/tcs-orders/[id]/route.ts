...
export async function GET(
  ...
  try {
    ...
    const tcsOrder = await TCSOrder.findById(params.id)
      .populate('order');
...
export async function PUT(
  ...
  try {
    ...
    return NextResponse.json(
      { error: 'Failed to update TCS order', details: error instanceof Error ? error.message : 'Unknown error' },
      { status: 500 }
    );
  }
}
...
async function handleTrackOrder(tcsOrder: any) {
  try {
    ...
    tcsOrder.apiErrors.push(`Track failed: ${error instanceof Error ? error.message : 'Unknown error'}`);
  }
}
...
async function handleCancelOrder(tcsOrder: any, actionData: any) {
  try {
    ...
    if (tcsOrder.order) {
      ...
      tcsOrder.order.cancellationReason = actionData.reason || 'Cancelled via TCS';
      tcsOrder.order.history.push({
        status: ORDER_STATUS.CANCELLED,
        changedAt: new Date(),
        changedBy: 'admin',
        reason: `TCS order cancelled: ${actionData.reason || 'No reason provided'}`
      });
    ...
    tcsOrder.trackingHistory.push({
      ...
      location: '',
      timestamp: new Date(),
      description: `Order cancelled: ${actionData.reason || 'No reason provided'}`,
      updatedBy: 'admin'
    });
  } catch (error) {
    console.error('Failed to cancel TCS order:', error);
    tcsOrder.apiErrors.push(`Cancel failed: ${error instanceof Error ? error.message : 'Unknown error'}`);
  }
}
...
async function handleGetPickupStatus(tcsOrder: any) {
  try {
    ...
    const pickupResponse = await tcsService.getPickupStatus(tcsOrder.consignmentNumber);
    
    tcsOrder.lastApiCall = new Date();
    tcsOrder.pickupStatus = pickupResponse.returnStatus?.status === 'SUCCESS' ? 'picked_up' : 'pending';
    
    tcsOrder.trackingHistory.push({
      status: 'pickup_checked',
      location: '',
      timestamp: new Date(),
      description: `Pickup status checked: ${pickupResponse.returnStatus?.message || 'No message'}`,
      updatedBy: 'admin'
    });
  } catch (error) {
    console.error('Failed to get pickup status:', error);
    tcsOrder.apiErrors.push(`Pickup status check failed: ${error instanceof Error ? error.message : 'Unknown error'}`);
  }
}
...
async function handleGetPaymentDetails(tcsOrder: any) {
  try {
    ...
    const paymentResponse = await tcsService.getPaymentDetails(tcsOrder.consignmentNumber);
    ...
    tcsOrder.trackingHistory.push({
      ...
      location: '',
      timestamp: new Date(),
      description: `Payment details checked: ${paymentResponse.returnStatus?.message || 'No message'}`,
      updatedBy: 'admin'
    });
  } catch (error) {
    console.error('Failed to get payment details:', error);
    tcsOrder.apiErrors.push(`Payment details check failed: ${error instanceof Error ? error.message : 'Unknown error'}`);
  }
}
