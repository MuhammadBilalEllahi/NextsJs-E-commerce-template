...
export async function POST(req: Request) {
  ...
  try {
    ...
    const raw: {
      ...
      showDescription:
        formData.get("showDescription") === "true" ? true : false,
    ...
    if (
      ...
      formData.get("expiresAt") !== "" &&
      formData.get("expiresAt") !== null &&
      formData.get("expiresAt") !== undefined
    ) {
    ...
    const uploadedImages = await uploaderFiles("banners", image, banner.id);

    const uploadImageToBanner = await Banner.findByIdAndUpdate(
      banner.id,
      {
...
export async function PATCH(req: Request) {
  try {
    ...
    return NextResponse.json(
      { error: "Failed to purge Redis cache" },
      { status: 500 }
    );
  }
}
...
export async function PUT(req: Request) {
  ...
  try {
    ...
    if (image && image.size > 0) {
      const uploadedImages = await uploaderFiles("banners", image, id);
      updateData = {
        ...updateData,
        image: uploadedImages[0].url,
        mimeType: uploadedImages[0].mimetype,
...
export async function DELETE(req: Request) {
  ...
  try {
    ...
    const deletedBanner = await Banner.findByIdAndDelete(id, { session });
