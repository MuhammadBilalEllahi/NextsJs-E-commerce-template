  ...
  try {
    ...
    const formattedOrders = orders.map(
      (order: {
        ...
        user: { firstName: any; lastName: any; email: any };
        shippingAddress: { firstName: any; lastName: any };
        contact: { email: any };
        total: any;
        items: {
          product: { title: any };
...
export async function PUT(req: NextRequest) {
  try {
    ...
    const { orderId, status, tracking, cancellationReason, changedBy } =
      await req.json();
