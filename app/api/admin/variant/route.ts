...
export async function POST(req: Request) {
  ...
  try {
    ...
    if (images.length > 0) {
      const uploadedImages = await uploaderFiles(
        "variants",
        images,
        variant[0].id
      );
...
export async function PUT(req: Request) {
  ...
  try {
    ...
    if (formData.has("isOutOfStock"))
      updateData.isOutOfStock = formData.get("isOutOfStock") === "true";
    ...
    if (formData.has("slug")) {
      ...
      if (newSlug && newSlug !== updateData.slug) {
        ...
        const existingVariantBySlug = await Variant.findOne({
          slug: newSlug,
          id: { $ne: variantId },
        });
        if (existingVariantBySlug) {
