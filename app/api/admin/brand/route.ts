  ...
  try {
    ...
    if (logoFile && logoFile instanceof Blob) {
      ...
      const originalFilename =
        (logoFile as any).name || `file-${Date.now()}.jpg`;
...
export async function PATCH(req: Request) {
  ...
  try {
    ...
    // If brand is being deactivated, deactivate all products under this brand
    if (isActive === false) {
      const Product = mongoose.model("Product");
      await Product.updateMany({ brand: id }, { isActive: false }, { session });
    }
