...
export async function GET(req: NextRequest) {
  try {
    ...
    const methodsWithCosts = shippingMethods.map(
      (method: {
        defaultShippingFee: any;
        defaultTcsFee: any;
        defaultEstimatedDays: any;
        locations: any[];
        ...
        if (city && method.locations) {
          const location = method.locations.find(
            (loc: { city: string; isAvailable: any }) =>
              loc.city.toLowerCase() === city.toLowerCase() && loc.isAvailable
          );
