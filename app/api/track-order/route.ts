...
function formatOrder(order: any) {
    return {
        ...
        items: order.items.map((item: any) => {
            ...
            return {
                productTitle: item.product?.name || item.product?.slug || 'Unknown Product',
    ...
    try {
        ...
        if (order && order.items?.length > 0 && !order.items[0].product?.name) {
            ...
            if (orderWithoutLean && orderWithoutLean.items?.[0]?.product?.name) {
                console.log('Population worked without lean()')
        ...
        if (order && order.items?.length > 0) {
            ...
            for (const item of order.items) {
                if (item.product && !item.product.name && item.product.slug) {
                    console.log(`Product ${item.product.slug} missing name field`)
                }
            }
        }
        ...
        order.items?.forEach((item: any, idx: number) => {
            ...
            console.log('  Product ref:', item.product)
            console.log('  Variant ref:', item.variant)
            console.log('  Product type:', typeof item.product)
            console.log('  Variant type:', typeof item.variant)
            if (item.product) {
                console.log('  Product fields:', Object.keys(item.product))
            }
            if (item.variant) {
                console.log('  Variant fields:', Object.keys(item.variant))
            }
        })
