...
export async function POST(req: Request) {
    ...
    try {
        ...
        // Cache the cart
        const cacheKey = body.user ? CACHE_CART_KEY(body.user._id) : CACHE_CART_KEY(body.uuidv4);
        await RedisClient.set(cacheKey, JSON.stringify(cart), CACHE_CART_EXPIRE_TIME.ONE_HOUR);
...
console.log(userId, sessionId);
        ...
        const cacheKey = userId ? CACHE_CART_KEY(userId) : CACHE_CART_KEY(sessionId!);
...
export async function PATCH(req: Request) {
    ...
    try {
        ...
        // Find guest cart
        const guestCart = await Cart.findOne({ uuidv4: sessionId }).session(session);
        if (!guestCart) {
            return NextResponse.json({ error: "Guest cart not found" }, { status: 404 });
        }

        // Find or create user cart
        let userCart = await Cart.findOne({ "user._id": userId }).session(session);
        ...
        await RedisClient.set(userCacheKey, JSON.stringify(userCart), CACHE_CART_EXPIRE_TIME.ONE_HOUR);
        await RedisClient.del(guestCacheKey);

        return NextResponse.json({ success: true, cart: userCart }, { status: 200 });
    } catch (err: any) {
        if (session) {
            await session.abortTransaction();
        }
        console.error("Error merging carts:", err);
        return NextResponse.json({ error: err.message || "Failed to merge carts" }, { status: 500 });
    } finally {
        session.endSession();
    }
}