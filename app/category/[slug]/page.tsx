...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> {
  ...
  if (slug === "all") {
    return {
      ...
      description: "Explore our complete collection of authentic Indian spices, pickles, and snacks at Dehli Mirch.",
    }
  }
  ...
  return {
    ...
    description: category.description || `Discover our premium ${category.name.toLowerCase()} collection at Dehli Mirch.`,
  }
}
...
export default async function CategoryPage({ params, searchParams }: CategoryPageProps) {
  ...
  const category = allCategories.find(cat => cat.name.toLowerCase() === slug.toLowerCase())
  ...
  const filteredProducts = filterProductsOnServer(allProducts, slug, searchParamsResolved)
...
function filterProductsOnServer(
  ...
  // Filter by category
  if (slug !== "all") {
    filtered = filtered.filter(p => p.category && p.category.toLowerCase() === slug.toLowerCase())
  }
  ...
  if (type) {
    filtered = filtered.filter((p) => p.type && p.type.toLowerCase() === type.toLowerCase())
  }
