...
export function BranchEditModal({
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      const def = countries.find(
        (c) =>
          c.label.toLowerCase() ===
          (formData.country || DEFAULT_COUNTRY).toLowerCase()
      );
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      // Try to keep existing state selection
      const curr = states.find(
        (s) => s.label.toLowerCase() === (formData.state ?? "").toLowerCase()
      );
      const def =
        curr ||
        states.find(
