...
export const createBrand = async (brand: { name: string; description: string; logo: File | null }) => {
...
// update brand
export const updateBrand = async (id: string, brand: { name: string; description: string; logo: File | null }) => {
    if (!brand.name) return
    const fd = new FormData()
    fd.append("id", id)
    fd.append("name", brand.name)
