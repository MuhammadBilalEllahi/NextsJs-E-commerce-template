...
type Action =
  | { type: typeof CartActionTypes.ADD; payload: { item: Omit<CartItem, "qty">; qty: number } }
  | { type: typeof CartActionTypes.REMOVE; payload: { id: string } }
  | { type: typeof CartActionTypes.UPDATE_QTY; payload: { id: string; qty: number } }
...
export function CartProvider({ children }: { children: React.ReactNode }) {
      ...
      const response = await fetch(`/api/cart?${params.toString()}`, {
        ...
        body: JSON.stringify({
          items: items.map(item => {
            ...
            console.log("Syncing item to backend:", mappedItem);
            console.log("Original variantLabel:", item.variantLabel);
            return mappedItem;
          }),
          userId,
          sessionId
  ...
  //   console.log("CartProvider: variantLabel:", item.variantLabel);
    
  //   setIsAdding(true)
    
  //   // Optimistic update
  //   dispatch({ type: CartActionTypes.ADD, payload: { item, qty } })
  //     setIsAdding(false)
  //   // Sync with backend
  //   // try {
...
//     // The commented-out sync code is where the backend call should happen.
//     // For now, let's assume it's commented for a reason and focus on the UI update.
//     // await syncCartWithBackend([...state.items, { ...item, qty }], 'add');

//   } catch (error) {
//     console.error("Failed to add item:", error);
//     // You might want to revert the optimistic update here on failure
//   } finally {
//     setIsAdding(false);
//   }
// };
const add = async (item: Omit<CartItem, "qty">, qty = 1) => {
  ...
  console.log("CART PROVIDER",count)

  return (
    <CartCtx.Provider value={{ openCartSheet,setOpenCartSheet,items: state.items, isHydrated,add, remove, updateQty, clear, count, subtotal, isAdding, isSyncing: state.isSyncing, refreshCart , syncAll }}>
      {children}
    </CartCtx.Provider>
  )
}
