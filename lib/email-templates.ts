...
export function generateOrderConfirmationEmail(data: OrderEmailData): string {
  const baseUrl = process.env.WEBSITE_URL || 'https://dehlimirchmasalajaat.com/';
    ...
    <div style="max-width: 600px; margin: 0 auto; background-color: #ffffff; border-radius: 12px; overflow: hidden; box-shadow: 0 4px 6px -1px rgba(0, 0, 0, 0.1);">
        ...
        <div style="padding: 32px 24px;">
            ...
            <div style="background-color: #f8f9fa; border: 1px solid #e2e8f0; border-radius: 8px; padding: 20px; margin: 24px 0;">
                ...
                <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 8px;">
                    ...
                    <span style="font-size: 14px; color: #1a1a1a; font-weight: 600;">#${data.orderId}</span>
                </div>
                <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 8px;">
                    <span style="font-size: 14px; color: #6b7280; font-weight: 500;">Reference ID:</span>
                    <span style="font-size: 14px; color: #1a1a1a; font-weight: 600;">#${data.refId}</span>
                </div>
                <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 8px;">
                    <span style="font-size: 14px; color: #6b7280; font-weight: 500;">Order Date:</span>
                    <span style="font-size: 14px; color: #1a1a1a; font-weight: 600;">${data.orderDate}</span>
                </div>
                <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 8px;">
                ...
                <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 8px;">
                    ...
                    <span style="font-size: 14px; color: #1a1a1a; font-weight: 600;">${data.paymentMethod}</span>
                </div>
            </div>
            ...
            <div style="margin: 24px 0;">
                ...
                ${data.items.map(item => `
                    <div style="display: flex; align-items: center; gap: 12px; padding: 12px 0; border-bottom: 1px solid #e2e8f0;">
                        <img src="${item.image || '/placeholder.svg'}" alt="${item.title}" style="width: 60px; height: 60px; border-radius: 8px; object-fit: cover; background-color: #f3f4f6;" />
                        <div style="flex: 1;">
                            <div style="font-size: 14px; font-weight: 600; color: #1a1a1a; margin-bottom: 4px;">${item.title}</div>
                            ${item.variantLabel ? `<div style="font-size: 12px; color: #6b7280; margin-bottom: 2px;">${item.variantLabel}</div>` : ''}
                            <div style="font-size: 12px; color: #6b7280;">Quantity: ${item.quantity}</div>
                        </div>
                        <div style="font-size: 14px; font-weight: 600; color: #1a1a1a;">${formatCurrency(item.priceAtPurchase * item.quantity)}</div>
                    </div>
                `).join('')}
            </div>
            ...
            <div style="background-color: #f8f9fa; border: 1px solid #e2e8f0; border-radius: 8px; padding: 20px; margin: 24px 0;">
                ...
                <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 8px;">
                    <span style="font-size: 14px; color: #6b7280;">Subtotal (${data.items.length} items)</span>
                    <span style="font-size: 14px; color: #1a1a1a; font-weight: 500;">${formatCurrency(data.subtotal)}</span>
                </div>
                <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 8px;">
                    <span style="font-size: 14px; color: #6b7280;">Shipping</span>
                    <span style="font-size: 14px; color: #1a1a1a; font-weight: 500;">${data.shippingFee === 0 ? 'FREE' : formatCurrency(data.shippingFee)}</span>
                </div>
                ${data.tcsFee && data.tcsFee > 0 ? `
                    <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 8px;">
                        <span style="font-size: 14px; color: #6b7280;">TCS Charges</span>
                        <span style="font-size: 14px; color: #1a1a1a; font-weight: 500;">${formatCurrency(data.tcsFee)}</span>
                    </div>
                ` : ''}
                <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 0; padding-top: 8px; border-top: 1px solid #e2e8f0; font-weight: 600; font-size: 16px;">
                    ...
                    <span style="color: #1a1a1a; font-weight: 700; font-size: 16px;">${formatCurrency(data.total)}</span>
                </div>
            </div>
            ...
            <div style="text-align: center; margin: 32px 0;">
                <a href="${baseUrl}/track-order?orderId=${data.orderId}" style="display: inline-block; background-color: #dc2626; color: #ffffff; text-decoration: none; padding: 12px 24px; border-radius: 8px; font-weight: 600; font-size: 16px;">
                    Track Your Order
                </a>
            </div>
        ...
        <div style="background-color: #f8f9fa; padding: 24px; text-align: center; border-top: 1px solid #e2e8f0;">
            ...
            <div style="font-size: 12px; color: #9ca3af; margin-top: 16px;">
                This email was sent to ${data.email}. If you have any questions, please contact us at support@dehlimirchmasalajaat.com
            </div>
        </div>
    </div>
</body>
