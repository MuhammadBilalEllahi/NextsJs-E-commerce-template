    ...
    for (const item of order.items) {
      if (item.variant) {
        try {
          ...
          if (result) {
            console.debug(
              `Restored ${item.quantity} units to variant ${item.variant}. New stock: ${result.stock}`
            );
          } else {
            console.error(
              `Failed to restore stock for variant ${item.variant}`
            );
          }
        } catch (error) {
          console.error(
            `Error restoring stock for variant ${item.variant}:`,
            error
          );
          throw error;
        }
        ...
        console.debug(
          `Product ${item.product} has no variant stock to restore`
        );
      }
    }

    console.debug(`Stock restored for cancelled order ${orderId}`);
  } catch (error) {
      ...
      if (item.variant) {
        try {
          ...
          if (result) {
            ...
            console.debug(
              `Decreased ${item.quantity} units from variant ${item.variant}. New stock: ${result.stock}`
            );
          } else {
            console.error(
              `Failed to decrease stock for variant ${item.variant}`
            );
          }
        } catch (error) {
          console.error(
            `Error decreasing stock for variant ${item.variant}:`,
            error
          );
          throw error;
        }
      } else {
        console.debug(`Item has no variant, skipping stock decrease`);
      }
    }
    ...
    for (const item of orderItems) {
      if (item.variantId) {
        ...
        if (variant.stock < item.quantity) {
          errors.push(
            `Insufficient stock for ${variant.label}. Available: ${variant.stock}, Required: ${item.quantity}`
          );
        }
      }
    }
